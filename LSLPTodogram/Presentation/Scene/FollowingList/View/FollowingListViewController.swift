...
final class FollowingListViewController: BaseViewController {
    ...
    private let mainView = FollowingListMainView()

    private let viewModel: FollowingListViewModel

    init(_ viewModel: FollowingListViewModel) {
...
//        let pull = mainView.tableView.refreshControl!.rx.controlEvent(.valueChanged).asObservable()
        ...
        let input = FollowingListViewModel.Input(
        ...
        output.items
            .drive(mainView.tableView.rx.items) { tv, row, item in
                guard let cell = tv.dequeueReusableCell(
                    withIdentifier: FollowingListCell.identifier
                ) as? FollowingListCell else {return UITableViewCell()}
